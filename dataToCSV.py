...
from data import DESC_JSON_PATH, DESC_CSV_PATH, CONTENT_CSV_PATH, CONTENT_JSON_PATH, COMMENT_CSV_PATH, COMMENT_JSON_PATH, REVIEW_ROOT_JSON_PATH, CRITIQUE_CSV_PATH, CRITIQUE_JSON_PATH, REVIEW_ROOT_CSV_PATH


def get_critique_row(_file_name, _path):
    """
    get critique row to create csv
    """
    print("critique csv --> %s" % _file_name)
    critique_json_path = _path + _file_name
...
def get_comment_row(_file_name, _path):
    """
    get comment row to create csv
    """
    print("comment csv --> %s" % _file_name)
    comment_json_path = _path + _file_name
...
def export_critique(_json_path, _csv_path):
    ...
    for file_name in critique_files:
        rows = get_critique_row(file_name, _json_path)
...
def export_desc(_json_path, _csv_path):
    ...
    for file_name in desc_files:
        row = get_desc_row(file_name, _json_path)
        rows.append(row)
...
def export_content(_json_path, _csv_path):
    ...
    for file_name in content_files:
        rows = get_content_row(file_name, _json_path)
        csv_rows.extend(rows)
...
def export_comment(_json_path, _csv_path):
    ...
    for file_name in comment_files:
        rows = get_comment_row(file_name, _json_path)
        csv_rows.extend(rows)
